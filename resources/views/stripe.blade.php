...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="{{URL::asset('css/example3.css')}}" data-rel-css="" />
  ...
  if (result.value) {
    Swal.fire(
      'Your Registration Will Be Locked After Payment',
      '',
      'warning'
    )
  }
...
<body onload="rules()">
      ...
      <div class="cell example example3" id="example-3">
        <form method="post" action="{!!route('addmoney.stripe')!!}">
          ...
          <div class="fieldset">
            ...
            <input id="example3-zip" data-tid="elements_examples.form.postal_code_placeholder" class="field empty third-width" type="hidden" value="{{Auth::user()->id}}">
          </div>
          <button type="submit" data-tid="elements_examples.form.pay_button">Pay {{ $college->totalfee }}</button>
  ...
  <script src="{{URL::asset('js/example3.js')}}" data-rel-js></script>
